...
public class Stamp implements Cloneable, Serializable {
     ...
     */
    public boolean equals(final Stamp s, final boolean creationTime, final boolean ocurrenceTime,
            final boolean evidentialBase) {
        if (this == s)
            return true;

        if (creationTime)
            if (getCreationTime() != s.getCreationTime())
                return false;
        if (ocurrenceTime)
            if (getOccurrenceTime() != s.getOccurrenceTime())
                return false;
        if (evidentialBase) {
