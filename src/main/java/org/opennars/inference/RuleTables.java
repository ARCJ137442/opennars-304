...
public class RuleTables {
    ...
    private static void applyRuleTable(TaskLink tLink, TermLink bLink, DerivationContext nal, Task task,
            Sentence taskSentence, Term taskTerm, Term beliefTerm, Sentence belief) {
                ...
                switch (bLink.type) {
                    ...
                    case TermLink.COMPONENT_CONDITION:
                        if ((belief != null) && (taskTerm instanceof Implication)) {
                            ...
                            SyllogisticRules.conditionalDedInd(task.sentence, (Implication) taskTerm, bIndex,
                    ...
                    case TermLink.COMPOUND_CONDITION:
                        if ((belief != null) && (beliefTerm instanceof Implication)) {
                            bIndex = bLink.getIndex(1);
                            SyllogisticRules.conditionalDedInd(belief, (Implication) beliefTerm, bIndex, taskTerm,
                                    tIndex, nal);
                        }
                        break;
                }
                ...
                switch (bLink.type) {
                    case TermLink.COMPOUND:
                        if (taskTerm instanceof CompoundTerm && beliefTerm instanceof CompoundTerm) {
                            compoundAndCompound((CompoundTerm) taskTerm, (CompoundTerm) beliefTerm, tIndex, bIndex,
                                    nal);
                        }
                        break;
                    case TermLink.COMPOUND_STATEMENT:
                        ...
                        if (belief != null) {
                            if (beliefTerm instanceof Implication) {
                                ...
                                if (Variables.unify(nal.memory.randomNumber, VAR_INDEPENDENT,
                                        ((Statement) beliefTerm).getSubject(), taskTerm, u, true)) { // only secure
                                                                                                     // place that
                                    ...
                                    SyllogisticRules.conditionalDedInd(belief, (Implication) beliefTerm, bIndex,
                                            taskTerm, -1, nal);
                                }
                ...
                switch (bLink.type) {
                    case TermLink.COMPONENT:
                        if (taskTerm instanceof Statement) {
                            ...
                            componentAndStatement((CompoundTerm) nal.getCurrentTerm(), bIndex, (Statement) taskTerm,
                                    tIndex, nal);
                        }
                        break;
                    case TermLink.COMPOUND:
                    ...
                    case TermLink.COMPOUND_CONDITION:
                        if (belief != null) {
                            ...
                            if ((taskTerm instanceof Statement) && (beliefTerm instanceof Implication)) {
                                conditionalDedIndWithVar(belief, (Implication) beliefTerm, bIndex, (Statement) taskTerm,
                                        tIndex, nal);
                            }
                        }
                        break;
                        ...
                        if (belief != null) {
                                                                 ...
                                                                 // conditionalDedIndWithVar()
                            {
                                ...
                                if (subj instanceof Negation) {
                                    ...
                                    conditionalDedIndWithVar(task.sentence, (Implication) taskTerm, tIndex,
                                            (Statement) beliefTerm, bIndex, nal);
                                }
                            }
                            break;
                        }
    ...
    public static void goalFromWantBelief(final Task task, final short tIndex, final short bIndex, final Term taskTerm,
            final DerivationContext nal, final Term beliefTerm) {
        if (task.sentence.isJudgment() && tIndex == 0 && bIndex == 1 && taskTerm instanceof Operation) {
            ...
            if (op.getPredicate() == nal.memory.getOperator("^want")) {
                final TruthValue newTruth = TruthFunctions.deduction(task.sentence.truth, nal.narParameters.reliance,
                        nal.narParameters);
                nal.singlePremiseTask(((Operation) taskTerm).getArguments().term[1], Symbols.GOAL_MARK, newTruth,
    ...
    private static void goalFromQuestion(final Task task, final Term taskTerm, final DerivationContext nal) {
        if (task.sentence.punctuation == Symbols.QUESTION_MARK
            ...
            final TruthValue truth = new TruthValue(1.0f,
     ...
     */
    private static void syllogisms(final TaskLink tLink, final TermLink bLink, final Term taskTerm,
            final Term beliefTerm, final DerivationContext nal) {
     ...
     * @param nal          Reference to the memory
     */
    private static void asymmetricAsymmetric(final Sentence taskSentence, final Sentence belief, final int figure,
        ...
        switch (figure) {
            case 11: // induction
            {
                final boolean sensational = SyllogisticRules.abdIndCom(beliefStatement.getPredicate(),
            ...
            case 22: // abduction
            {
                if (!SyllogisticRules.conditionalAbd(taskStatement.getSubject(), beliefStatement.getSubject(),
                    ...
                    final boolean sensational = SyllogisticRules.abdIndCom(taskStatement.getSubject(),
            ...
            case 21: // exemplification
                ...
                if (Variables.unify(nal.memory.randomNumber, VAR_QUERY, t1, t2,
                        new Term[] { taskStatement, beliefStatement })) {
     ...
     * @param nal    Reference to the memory
     */
    private static void asymmetricSymmetric(final Sentence asym, final Sentence sym, final int figure,
            final DerivationContext nal) {
     ...
     * @param nal          Reference to the memory
     */
    private static void symmetricSymmetric(final Sentence belief, final Sentence taskSentence, final int figure,
            final DerivationContext nal) {
     ...
     * @param nal                  Reference to the memory
     */
    private static void detachmentWithVar(final Sentence originalMainSentence, final Sentence subSentence,
            final int index, final DerivationContext nal) {
        detachmentWithVar(originalMainSentence, subSentence, index, true, nal);
    }

    private static void detachmentWithVar(final Sentence originalMainSentence, Sentence subSentence, final int index,
            final boolean checkTermAgain, final DerivationContext nal) {
     ...
     * @param nal         Reference to the memory
     */
    private static void conditionalDedIndWithVar(final Sentence conditionalSentence, Implication conditional,
            final short index, Statement statement, short side, final DerivationContext nal) {
     ...
     * @param nal          Reference to the memory
     */
    private static void compoundAndSelf(final CompoundTerm compound, final Term component, final boolean compoundTask,
            final int index, final DerivationContext nal) {
     ...
     * @param nal        Reference to the memory
     */
    private static void compoundAndCompound(final CompoundTerm taskTerm, final CompoundTerm beliefTerm,
            final int tindex, final int bindex, final DerivationContext nal) {
     ...
     * @param nal        Reference to the memory
     */
    private static void compoundAndStatement(CompoundTerm compound, final short index, Statement statement,
            final short side, final Term beliefTerm, final DerivationContext nal) {
     ...
     * @param nal       Reference to the memory
     */
    private static void componentAndStatement(final CompoundTerm compound, final short index, final Statement statement,
            final short side, final DerivationContext nal) {
        if (statement instanceof Inheritance) {
    ...
    public static void transformTask(final TaskLink tLink, final DerivationContext nal) {
        ...
        Term expectedInheritanceTerm = null; // we store here the (dereferenced) term which we expect to be a
                                             // inheritance
