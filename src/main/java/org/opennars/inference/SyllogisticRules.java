...
public final class SyllogisticRules {
     ...
     */
    static void dedExe(final Term term1, final Term term2, final Sentence sentence, final Sentence belief,
     ...
     * @param nal       Reference to the memory
     */
    static boolean abdIndCom(Term term1, Term term2, final Sentence sentence1, final Sentence sentence2,
            final int figure, final DerivationContext nal) {
     ...
     * @param nal    Reference to the memory
     */
    static void analogy(final Term subj, final Term pred, final Sentence asym, final Sentence sym, final int figure,
            final DerivationContext nal) {
     ...
     * @param nal      Reference to the memory
     */
    static void resemblance(final Term term1, final Term term2, final Sentence belief, final Sentence sentence,
            final int figure, final DerivationContext nal) {
        if (Statement.invalidStatement(term1, term2)) {
            return;
        }
        ...
        final boolean bothHigherOrder = (belief.term.isHigherOrderStatement()
        ...
        final Statement s = Statement.make(higherOrder ? NativeOperator.EQUIVALENCE : NativeOperator.SIMILARITY, term1,
                term2, order);
     ...
     * @param nal          Reference to the memory
     */
    static void detachment(final Sentence mainSentence, final Sentence subSentence, final int side,
            final DerivationContext nal) {
     ...
     * @param nal      Reference to the memory
     */
    static void conditionalDedInd(final Sentence premise1Sentence, Implication premise1, short index, Term premise2,
            final int side, final DerivationContext nal) {
        final Task task = nal.getCurrentTask();
        final Sentence taskSentence = task.sentence;
        final Sentence belief = nal.getCurrentBelief();
        final boolean deduction = (side != 0);
        final boolean conditionalTask = Variables.hasSubstitute(nal.memory.randomNumber, Symbols.VAR_INDEPENDENT,
                premise2, belief.term);
        final Term commonComponent;
        Term newComponent = null;
        if (side == 0 || side == 1) {
            Statement.EnumStatementSide sideOfCommonComponentAsEnum = side == 0 ? Statement.EnumStatementSide.SUBJECT
        ...
        if (index2 >= 0) {
            ...
            boolean match = Variables.unify(nal.memory.randomNumber, Symbols.VAR_INDEPENDENT, oldCondition.term[index],
                    commonComponent, u);
        ...
        if (newCondition != null) {
            if (newCondition instanceof Interval) {
                ...
                while ((newCondition instanceof Conjunction)
                        && (((CompoundTerm) newCondition).term[0] instanceof Interval)) {
     ...
     * @param nal      Reference to the memory
     */
    static void conditionalAna(Equivalence premise1, final short index, Term premise2, final int side,
            final DerivationContext nal) {
        final Task task = nal.getCurrentTask();
        final Sentence taskSentence = task.sentence;
        final Sentence belief = nal.getCurrentBelief();
        ...
        Term[] u = new Term[] { premise1, premise2 };
        boolean match = Variables.unify(nal.memory.randomNumber, Symbols.VAR_DEPENDENT, oldCondition.term[index],
                commonComponent, u);
        ...
        if (!match && (commonComponent.getClass() == oldCondition.getClass())) {
            ...
            match = Variables.unify(nal.memory.randomNumber, Symbols.VAR_DEPENDENT, oldCondition.term[index],
                    ((CompoundTerm) commonComponent).term[index], u);
            premise1 = (Equivalence) u[0];
            premise2 = u[1];
        }
        if (!match) {
     ...
     */
    static boolean conditionalAbd(final Term cond1, final Term cond2, final Statement st1, final Statement st2,
            final DerivationContext nal) {
        ...
        final boolean keepOrder = Variables.hasSubstitute(nal.memory.randomNumber, Symbols.VAR_INDEPENDENT, st1,
                task.getTerm());
     ...
     * @param nal          Reference to the memory
     */
    static void elimiVarDep(final CompoundTerm compound, final Term component, final boolean compoundTask,
            final DerivationContext nal) {
        Term comp = null;
