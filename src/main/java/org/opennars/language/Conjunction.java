...
public class Conjunction extends CompoundTerm {
    ...
    protected Conjunction(final Term[] arg, final int order, final boolean normalized, final boolean spatial,
            final CompoundTerm.ConvRectangle rect) {
        super(arg);
    ...
    public static Term[] flatten(final Term[] args, final int order, final boolean isSpatial) { // flatten only same
                                                                                                // order!
    ...
    public static Term UpdateRelativeIndices(final int minX, final int minY, final int minsX, final int minsY,
            final Term term) {
            ...
            for (final Term t : flattened) {
                if (!(t instanceof Interval)) { // intervals only for seqs
                    if (t.term_indices == null || rect == null || rect.term_indices == null) {
                        ...
                        final Term updated = UpdateRelativeIndices(rect.term_indices[2], rect.term_indices[3],
