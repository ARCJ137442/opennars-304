...
public class Emotions implements Plugin, Serializable {
    ...
    public volatile float BUSY_EVENT_HIGHER_THRESHOLD = 0.9f; // 1.6.4, step by step^, there is already enough new
                                                              // things ^^
    ...
    public void adjustSatisfaction(final float newValue, final float weight, final DerivationContext nal) {
        ...
        if (Math.abs(happy - lasthappy) > CHANGE_THRESHOLD
                && nal.time.time() - last_happy_time > CHANGE_STEPS_DEMANDED) {
        ...
        if (frequency != -1) { // ok lets add an event now
            ...
            final TruthValue truth = new TruthValue(happy, nal.narParameters.DEFAULT_JUDGMENT_CONFIDENCE,
                    nal.narParameters);
    ...
    public void adjustBusy(final float newValue, final float weight, final DerivationContext nal) {
        ...
        if (frequency != -1) { // ok lets add an event now
            ...
            final Inheritance inh = Inheritance.make(subject, predicate);
            final TruthValue truth = new TruthValue(busy, nal.narParameters.DEFAULT_JUDGMENT_CONFIDENCE,
                    nal.narParameters);
