...
public class InternalExperience implements Plugin, EventObserver {
    ...
    @Override
    public boolean setEnabled(final Nar n, final boolean enable) {
        memory = n.memory;
        this.nar = n;
    ...
    public void event(final Class<?> event, final Object[] a) {
        ...
        if (event == Events.ConceptDirectProcessedTask.class) {
            ...
            if (OLD_BELIEVE_WANT_EVALUATE_WONDER_STRATEGY || (!OLD_BELIEVE_WANT_EVALUATE_WONDER_STRATEGY
    ...
    public static boolean InternalExperienceFromTaskInternal(final Memory memory, final Task task, final boolean full,
        ...
        final TruthValue truth = new TruthValue(1.0f, memory.narParameters.DEFAULT_JUDGMENT_CONFIDENCE,
    ...
    protected void beliefReason(final Sentence belief, final Term beliefTerm, final Term taskTerm,
        ...
        if (nal.memory.randomNumber.nextDouble() < INTERNAL_EXPERIENCE_RARE_PROBABILITY) {
            ...
            final Operator op = memory.getOperator(
                    nonInnateBeliefOperators[nal.memory.randomNumber.nextInt(nonInnateBeliefOperators.length)]);
        ...
        if (beliefTerm instanceof Implication
                && nal.memory.randomNumber.nextDouble() <= INTERNAL_EXPERIENCE_PROBABILITY) {
