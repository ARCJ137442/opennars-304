...
public class VisionChannel extends SensoryChannel {
    ...
    final Term label;
    final Nar nar;
    boolean HadNewInput = false; // only generate frames if at least something was input since last "commit to
                                 // Nar"
    public final EventEmitter.EventObserver obs;

    public VisionChannel(final String label, final Reasoner nar, final Reasoner reportResultsTo, final int width,
            final int height, final int duration,
            ...
            if (this.prototypes.isEmpty()) {
                ...
                if (lastPrototype != null) {
                    ...
                    try {
                        ...
                        Task taskX = new Narsese(this.nar).parseTask("(^move,{SELF}," + minusX + Texts.n1(xParam) + ","
