...
public class VisualSpace implements ImaginationSpace {
    ...
    public VisualSpace(final Nar nar, final float[][] source, final int py, final int px, final int height,
            final int width) {
        this.nar = nar;
        this.height = height;
        this.width = width;
            ...
            for (int oi = -this.width; oi < this.width; oi++) {
                ...
                for (int i = 0; i < this.height; i++) {
                    for (int j = 0; j < this.width; j++) {
                        ...
                        TruthValue t1 = new TruthValue(cropped[transi][transj],
                                nar.narParameters.DEFAULT_JUDGMENT_CONFIDENCE, nar.narParameters);
                        TruthValue t2 = new TruthValue(other.cropped[i2][j2],
                                nar.narParameters.DEFAULT_JUDGMENT_CONFIDENCE, nar.narParameters);
                        TruthValue t3 = comparison ? TruthFunctions.comparison(t1, t2, nar.narParameters)
                                : TruthFunctions.abduction(t1, t2, nar.narParameters);
